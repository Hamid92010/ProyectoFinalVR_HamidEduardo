...
public class WeaponManager : MonoBehaviour
{
    ...
    void Update()
    {
        if (!GameManager.sharedInstance.isPaused && !GameManager.sharedInstance.isGameOver)
        {
            ...
            if (( OVRInput.Get(OVRInput.RawButton.LIndexTrigger)) && reserveAmmo > 0) //Input.GetKeyDown(KeyCode.R) ||
            {
            ...
            if (( OVRInput.Get(OVRInput.RawButton.RIndexTrigger)) && fireRateTimer <= 0 && isAutomatic) //Input.GetButton("Fire1") ||
            {
                Shoot();
                fireRateTimer = 1 / fireRate;
            }

            if (( OVRInput.Get(OVRInput.RawButton.RIndexTrigger)) && fireRateTimer <= 0 && !isAutomatic) //Input.GetButtonDown("Fire1") ||
            {
                Shoot();
                fireRateTimer = 1 / fireRate;
            }

            if (OVRInput.GetDown(OVRInput.Button.PrimaryThumbstick)) //Input.GetButtonDown("Fire2") || 
            {
                Aim();
            }

            if ( OVRInput.GetUp(OVRInput.Button.PrimaryThumbstickUp)) //Input.GetButtonUp("Fire2") ||
            {
